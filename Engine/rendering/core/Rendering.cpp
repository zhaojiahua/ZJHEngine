...
{
	if (ZEngineWind* engineWnd=static_cast<ZEngineWind*>(engine)) {
...
ComPtr<ID3D12Resource> IRenderingInterface::ConstructDefaultBuffer(ComPtr<ID3D12Resource>& upTempBuffer, const void* indata, UINT64 indatasize)
{
	...
	CD3DX12_HEAP_PROPERTIES tempProperties = CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_DEFAULT);//描述符堆
	...
	CD3DX12_HEAP_PROPERTIES updataBufferProperties = CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_DEFAULT);//描述符堆
	...
	UpdateSubresources<1>(GetdcommandList().Get(), tempbuffer.Get(), upTempBuffer.Get(), 0, 0, 1, &subresourceData);
	//切换资源状态
